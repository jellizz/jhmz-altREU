...
SUMMARY_CSV = OUTPUT_DIR / "updated_h_index_citation_summary.csv"

CITATION_LEVEL_CSV = (
    OUTPUT_DIR / "updated_h_index_citation_level_data.csv"
)
...
# ============================================================
# H-INDEX BY REFERENCE TYPE
# ============================================================

def analyze_h_index_by_reference_type(citations):

    """
    Analyze h-index availability separately for:

        found      = non-hallucinated / verified reference
        not_found  = hallucinated reference
        other      = other/unclassified reference status

    The h-index belongs to the identified author associated with
    the citation. These statistics therefore tell us how much
    author-productivity information was available within each
    reference-status group.
    """
    ...
    for reference_type in [
        ...
        results[
            f"{prefix}_found_pct_identified"
        ] = calculate_pct(
            len(h_indices),
            identified_author_count
        )
        ...
        results[
            f"{prefix}_identified_authors"
        ] = identified_author_count

        results[
            f"{prefix}_total_references"
        ] = total
...
def make_summary_row(
    ...
    row.update(
        analyze_h_index_by_reference_type(
            citations
        )
    )
