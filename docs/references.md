# References & Tools

## Research Papers  
- Zhao, Z., Wang, Y., Stuart, T., Vaan, M.D., Ginsparg, P.H., & Yin, Y. (2026). LLM hallucinations in the wild: Large-scale evidence from non-existent citations. https://doi.org/10.48550/arXiv.2605.07723.  

- Chakraborty, S., Chowdhury, R., Shuvo, S.R. et al. A scalable framework for evaluating multiple language models through cross-domain generation and hallucination detection. Sci Rep 15, 29981 (2025). https://doi.org/10.1038/s41598-025-15203-5.  

- Asai, A., He, J., Shao, R. et al. Synthesizing scientific literature with retrieval-augmented language models. Nature 650, 857–863 (2026). https://doi.org/10.1038/s41586-025-10072-4. 


## Tools & Software   
- [Hallucinator](https://github.com/gianlucasb/hallucinator) — Used to verify citation status.  

- [Genderize](https://genderize.io/) — Used to identify gender from name.  


## APIs & Data Sources  
- [OpenAlex](https://openalex.org/) — Used to gather abstracts.

## AI Models  
- OpenAI GPT-5.5
- Anthropic Claude-Opus-4.8
- Google Gemini-Flash-3.5

## Presentation Images
- Librarian finds ‘preposterous number’ of fake references in paper from Springer Nature journal 
  https://retractionwatch.com/2026/03/06/librarian-finds-preposterous-number-of-fake-references-in-paper-from-springer-nature-journal/  

- Atallah, M. RETRACTED ARTICLE: From Surgery to Strength: The Case for Mental Health Integration in Ileostomy Care. Dig Dis Sci 70, 2947–2950 (2025). https://doi.org/10.1007/s10620-025-09054-3